...
This is a template squid for indexing Acala EVM+ contracts deployed to the Acala or Karura networks. This template indexes `KAR` token transfers on the Karura Network and serves them via a GraphQL API.
...
It is necessary to import the respective ABI definition to decode EVM logs. For Moonsama transfers we use the standard ERC721 interface, see [`assets/ERC20.json`](assets/ERC20.json).

To generate a type-safe facade class to decode EVM logs, use `squid-evm-typegen(1)`:

```bash
npx squid-evm-typegen src/abi assets/ERC20.json#erc20
```
